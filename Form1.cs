    ...
    public partial class Form1 : Form
    {
        ...
        private void serialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)  //수신 이벤트가 발생하면 이 부분이 실행된다.
        {
            ...
            if (this.InvokeRequired)//UI쓰레드 충돌 없애기
            {
                ...
                {
                    for(byte i=0; i < byte2Reed; i++)
                    {
                    richTextBox_received.Text = richTextBox_received.Text + string.Format("{0:X2}", bufferr[i]);
                    }
                    if (tabControl1.SelectedIndex == 1)          BatteryInfoUpdate();
                }));
            }
